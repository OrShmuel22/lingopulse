...
- Runs a ping script: `curl -s http://127.0.0.1:11434/api/generate -d '{"model":"gemma3:1b-it-qat","prompt":" ","keep_alive":"30m","stream":false}'`.
...
3. **Thermal reality on M4 Air (fanless):** gemma3:1b-it-qat inference on short (<50 word) prompts should not sustain long enough to throttle. If longer refinements (emails) show degraded wall-clock time during sustained use, revisit quantization or model size. Not a v1 blocker.
...
| 2 | Ship login warmup LaunchAgent | Infrastructure | After `launchctl bootstrap`-ing the plist, logging out and back in results in Ollama showing `gemma3:1b-it-qat` loaded in `ollama ps` within 30 s of login, with no hotkey press. |
| 3 | Ship scheduled keep-alive LaunchAgent | Infrastructure | At 09:00 (inside active hours), `ollama ps` shows `gemma3:1b-it-qat` loaded. At 23:00 + 35 min (outside active hours + past keep-alive), `ollama ps` shows it unloaded. Interval and active hours honor `config.json`. |
