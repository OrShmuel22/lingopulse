...
@Suite struct IMEDebouncerTests {
    @Test func firesAfterInterval() async {
        let bgQueue = DispatchQueue(label: "ime-debouncer-test")
    ...
    @Test func cancelsPriorOnReschedule() async {
        let bgQueue = DispatchQueue(label: "ime-debouncer-test-cancel")
        let debouncer = Debouncer(interval: 0.05, queue: bgQueue)
    ...
    @Test func cancelStopsFire() async {
        let bgQueue = DispatchQueue(label: "ime-debouncer-test-stop")
        let debouncer = Debouncer(interval: 0.05, queue: bgQueue)
